...
function setPrinterToSuspended(card) {
  ...
  console.log(`✅ Printer "${card.querySelector(".demo-printer-name")?.textContent.trim()}" set to SUSPENDED`);
}
...
function setPrinterToError(card) {
  ...
  console.log(`✅ Printer "${card.querySelector(".demo-printer-name")?.textContent.trim()}" set to ERROR`);
}
...
if (printerName && printerDataMap.has(printerName)) {
  ...
  setTimeout(() => {
    ...
    console.log("finishPrint(printerCard, jobID)", printerCard, jobID);
  }, 500);
}
...
function findAndAssignNextJob(printerCard) {
  ...
  for (const job of jobDataMap.values()) {
    ...
    console.log("matchingPrinters", matchingPrinters, "printerName", printerName);
    console.log("job.jobProgQueued", job.jobProgQueued, "matchingPrinters.includes(printerName)", matchingPrinters.includes(printerName));
...
function updateTabWithID(tabID, fileName) {
  ...
  const fileData = fileDataMap.get(fileName);
  const tabDataID = fileName.replace(/\s+/g, "-").toLowerCase();
  if (!fileData) return;
  const tabFile = fileData.fileName;
  console.log("tabFile", tabFile, "tabID", tabID, "fileData", fileData);
  const tab = (tabID === tabFile) ? document.querySelector(`.file-tab[data-w-tab="${tabDataID}"]`) : document.querySelector(`.file-tab[data-tab-id="${tabDataID}"]`);
  const pane = (tabID === tabFile) ? document.querySelector(`.file-info-tab[data-w-tab="${tabDataID}"]`) : document.querySelector(`.file-info-tab[data-tab-id="${tabDataID}"]`);
  console.log(tab);
  console.log(pane);
