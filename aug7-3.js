...
if (!tagSearch.dataset.keyListenerAttached) {
  tagSearch.addEventListener('keydown', function (e) {
    ...
    if (!tagAlreadyExists && !matchesVisible) {
      ...
      // Clone the hidden modal template element and attach to the current tagWrap
const editModalTemplate = document.getElementById("edit-tag-modal-template");
let editModal = null;

if (editModalTemplate) {
  const editModalClone = editModalTemplate.cloneNode(true); // clone the full .edit-tag-modal div
  editModalClone.id = "";                                   // clear the ID to avoid duplication
  editModalClone.style.display = "none";                    // hide it until edit button is clicked
  tagWrap.appendChild(editModalClone);                      // attach to tag

  editModal = tagWrap.querySelector(".edit-tag-modal");     // store reference to use later
}

// Add edit button to the tag
const tagEdit = document.createElement("button");
tagEdit.className = "edit-tag-button";
tagEditWrap.appendChild(tagEdit);

// Hook up click event to show and populate modal
tagEdit.addEventListener("click", (e) => {
  ...
  // 1. Fill in tag name input
  const nameInput = editModal.querySelector('.tag-name-wrap .tag-edit-input');
  if (nameInput) {
    nameInput.value = tagName;
  }

  // 2. Fill in category input
  const categoryInput = editModal.querySelector('.tag-category-wrap .tag-edit-input');
  if (categoryInput) {
    let category = tagRepoMap.get(tagName) || "";

    // Default to Colour if known color and no category in map
    if (!category && isKnownColorTag(tagName)) {
      category = "Colour";
    }

    // Normalize color categories
    ...
    if (previewText) {
      previewText.textContent = category;
    }
  }

  // 3. Hook up color swatch click
  ...
  colorOptions.forEach(option => {
    option.addEventListener("click", () => {
      ...
      if (selectedColor) {
        tagEl.className = "tag-name-in-list"; // reset
        tagEl.classList.add(selectedColor);   // apply new color
      }
    });
  });

  // 🔁 NEW — Save/Cancel/Outside Click Behavior

  const originalName = tagName;
  ...
  if (saveBtn) {
    saveBtn.onclick = () => {
      ...
      if (newName) tagEl.textContent = newName;

      const previewText = editModal.querySelector(".tag-preview-text");
      if (previewText) {
        previewText.textContent = newCategory;
      }
  ...
  if (cancelBtn) {
    cancelBtn.onclick = () => {
      ...
      if (categoryInput) categoryInput.value = originalCategory;
      const previewText = editModal.querySelector(".tag-preview-text");
      if (previewText) previewText.textContent = normalizeCategory(originalCategory);

      editModal.style.display = 'none';
    };
  }

  // Background click closes modal and reverts
  ...
  if (background && !background.dataset.listenerAttached) {
    ...
    background.dataset.listenerAttached = "true"; // prevent rebinding
  }
});
      ...
      tagDel.addEventListener('click', (e) => {
        e.stopPropagation(); // Prevent tagWrap click

        const tag = tagEl.textContent.trim(); // normalize
        tagToDelete = tag; // ✅ Step 1: store the tag

        document.getElementById("tag-to-delete-name").textContent = tag; // ✅ Step 3
        document.getElementById("delete-tags-modal").style.display = "flex"; // ✅ Step 2
      });
      ...
      if (!tagSearch.dataset.keyListenerAttached) {
       tagSearch.addEventListener('keydown', function (e) {
          ...
          if (!tagAlreadyExists && !matchesVisible) {
            ...
            const tagWrap = document.createElement("div");
            tagWrap.className = "tag-in-list-wrap";
            tagWrap.style.display = "none"; // Will show again on input match
            tagListWrap.appendChild(tagWrap);
  ...
  if (!tagSearch.dataset.keyListenerAttached) {
    tagSearch.addEventListener('keydown', function (e) {
      ...
      if (!tagAlreadyExists && !matchesVisible) {
        ...
        const tagWrap = document.createElement("div");
        tagWrap.className = "tag-in-list-wrap";
        tagWrap.style.display = "none"; // hide until searched
        tagListWrap.appendChild(tagWrap);
