...
if (!tagSearch.dataset.keyListenerAttached) {
  tagSearch.addEventListener('keydown', function (e) {
    ...
    if (!tagAlreadyExists && !matchesVisible) {
      ...
      if (editModalTemplate) {
        const editModalClone = editModalTemplate.content.cloneNode(true);
        tagWrap.appendChild(editModalClone);
      }

      const editModal = tagWrap.querySelector('.edit-tag-modal');
  ...
  // 2. Fill in tag category input
  const categoryInput = editModal.querySelector('.tag-category-wrap .tag-edit-input');
  if (categoryInput) {
    const fromMap = tagRepoMap.get(tagName);
    let category = "";

    if (fromMap) {
      category = fromMap;
    } else if (isKnownColorTag(tagName)) {
      category = "Colour";
    }

    categoryInput.value = normalizeCategory(category);

    // Optional: update preview bubble
    const previewText = editModal.querySelector(".tag-preview-text");
    if (previewText) {
      previewText.textContent = normalizeCategory(category);
    }
  }

  // 3. Hook up color swatch selection
  const colorOptions = editModal.querySelectorAll(".tag-colour-option");
  colorOptions.forEach(option => {
    option.addEventListener("click", () => {
      const allClasses = Array.from(option.classList);
      const selectedColor = allClasses.find(cls => cls !== "tag-colour-option");

      if (selectedColor) {
        // Remove existing color classes from the tagEl
        tagEl.className = "tag-name-in-list";
