...
// Tag map
const globalTagMap = {
  color: new Set(["Red", "Blue", "White", "Black", "Grey", "Green", "Pink", "Orange", "Yellow"]),
  material: new Set(["PLA", "PETG", "ABS", "Matte PLA", "Silk PLA"]),
  printer: new Set([
    "Bambu A1", "Bambu A1 Mini", "Bambu X1C", "Bambu P1P", "Bambu P1S",
    "Prusa Mk3", "Prusa MK4", "Prusa Mini",
    "Ender 3", "Ender 3 V2", "CR 10"
  ])
};
...
function initializePrinterData() {
  ...
  printerCards.forEach((card, index) => {
    if (card.id === "printer-template") return;
...
function updateDefaultTagsUI(container, tagList) {
  ...
  const maxTags = 4;
  const overflowTagCount = tagList.length - maxTags;
  const visibleTags = tagList.slice(0, maxTags);

  visibleTags.forEach(tag => {
    const displayValue = tag.includes(":") ? tag.split(":")[1] : tag;
    const tagType = getTagType(displayValue);

    const div = document.createElement("div");
    div.className = `demo-tag${tagType === 'color' ? ` color-${displayValue.toLowerCase()}` : ''}`;
    div.style.display = "flex";

    const inner = document.createElement("div");
    inner.className = "demo-tag-text";
    inner.textContent = displayValue;
  ...
  if (overflowTagCount > 0) {
    ...
    overflow.className = "demo-tag overflow";
    overflow.textContent = `+${overflowTagCount}`;
...
function removeTagFromJob(jobID, tagValueOnly) {
  const job = jobDataMap.get(jobID);
  if (!job || !Array.isArray(job.tags)) return;

  const tagType = getTagType(tagValueOnly);
  const fullTag = `${tagType}:${tagValueOnly}`;

  if (job.tags.includes(fullTag)) {
    job.tags = job.tags.filter(t => t !== fullTag);
    jobDataMap.set(jobID, structuredClone(job));
    console.log("🗑️ Removed tag from job:", fullTag, job.tags);
  }
}

function addTagToModal(tagValue, container) {
  const exists = Array.from(container.querySelectorAll('.demo-tag.in-modal'))
    .some(el => el.textContent.trim() === tagValue);
  if (exists) return;

  const tagType = getTagType(tagValue);
  const fullTag = `${tagType}:${tagValue}`;

  const div = document.createElement("div");
  div.className = `demo-tag in-modal${tagType === 'color' ? ` color-${tagValue.toLowerCase()}` : ''}`;
  div.style.display = "flex";

  const inner = document.createElement("div");
  inner.className = "demo-tag-text in-modal";
  inner.textContent = tagValue;

  div.appendChild(inner);
  container.appendChild(div);

  // 🟡 Attach removal logic immediately
  div.addEventListener("click", () => {
    const modal = container.closest(".add-tag-modal");
    const fileName = modal?.getAttribute("data-file-id");
    const jobID = modal?.getAttribute("data-job-id");

    if (fileName) {
      removeTagFromFile(fileName, tagValue);
    } else if (jobID) {
      removeTagFromJob(jobID, tagValue);
    }

    removeTagFromModal(div, fullTag, container);
    ...
    const restoreWrap = Array.from(tagListWrap.querySelectorAll('.tag-in-list-wrap')).find(w =>
      w.querySelector('.tag-name-in-list')?.textContent.trim() === tagValue
    );
...
function removeTagFromModal(tagEl, fullTag, container) {
  const [type, value] = fullTag.split(":");
  tagEl.remove();
  ...
  if (fileName && fileDataMap.has(fileName)) {
    const fileData = fileDataMap.get(fileName);
    fileData.defaultTags = fileData.defaultTags.filter(t => t !== fullTag);
    fileDataMap.set(fileName, structuredClone(fileData));

    // Re-render default tags in all instances
    updateAllInstancesOfFile(fileName);
  }

  const listWrap = modal?.querySelector(".tag-list-wrap");
  if (listWrap) {
    const wrap = document.createElement("div");
    wrap.className = "tag-in-list-wrap";
    wrap.setAttribute("data-type", type);
    wrap.setAttribute("data-value", value);

    const tag = document.createElement("div");
    tag.className = "tag-name-in-list";
...
function initTagAndDistributeListeners() {
  ...
  document.querySelectorAll('.add-tag').forEach(addTagBtn => {
    ...
    addTagBtn.addEventListener('click', e => {
      ...
      populateTagListWrap(tagListWrap, printer.printerTags);
...
tagListWrap.querySelectorAll('.tag-name-in-list').forEach(tagEl => {
  ...
  const fullTag = `${tagType}:${tagValue}`; // e.g., "color:red"
  const tagWrap = tagEl.closest('.tag-in-list-wrap');

  tagWrap.style.display = job.tags.includes(fullTag) ? 'none' : 'flex';

  tagEl.onclick = () => {
    console.log("New tag clicked");

    addTagToJob(jobID, fullTag);
    addTagToModal(tagValue, currentTagsWrap);
    ...
    if (tagWrap) tagWrap.style.display = 'none';
      ...
      currentTagsWrap.onclick = (e) => {
        ...
        const tagValueOnly = clicked.textContent.trim();
const tagType = getTagType(tagValueOnly);
const fullTag = `${tagType}:${tagValueOnly}`;
removeTagFromJob(jobID, fullTag);
        removeTagFromModal(clicked, fullTag, currentTagsWrap);
        ...
        const restoreWrap = Array.from(tagListWrap.querySelectorAll('.tag-in-list-wrap')).find(w =>
          w.querySelector('.tag-name-in-list')?.textContent.trim() === tagValueOnly
        );
        if (restoreWrap) restoreWrap.style.display = 'flex';
      };
    });
...
function initPrinterTagListeners() {
  ...
  document.querySelectorAll('.add-tag.printer').forEach(addTagBtn => {
    ...
    printer.printerTags.forEach(tag => {
      const valueOnly = tag.includes(":") ? tag.split(":")[1] : tag;
      addTagToModal(valueOnly, currentTagsWrap);
    });
    ...
    addTagBtn.addEventListener('click', e => {
      ...
      // ✅ Dynamic list from globalTagMap
      populateTagListWrap(tagListWrap, printer.printerTags);
      ...
      tagListWrap.querySelectorAll('.tag-name-in-list').forEach(tagEl => {
        ...
        tagWrap.style.display = printer.printerTags.includes(fullTag) ? 'none' : 'flex';

        tagEl.onclick = () => {
          if (!printer.printerTags.includes(fullTag)) {
            printer.printerTags.push(fullTag);
            ...
            console.log(`➕ Added tag to printer: ${printerName}`, fullTag);
          }
        };
      });
...
if (printer.printerTags.includes(fullTag)) {
  printer.printerTags = printer.printerTags.filter(t => t !== fullTag);
          printer.printerTagsNum = printer.printerTags.length;
          printerDataMap.set(printerName, structuredClone(printer));

          removeTagFromModal(tagValueOnly, currentTagsWrap);
          updateDefaultPrinterTagsUI(printerCard.querySelector('.default-file-tags'), printer.printerTags);
          ...
          updateAllJobMatches();
          updatePrinterCompatibleJobs(printerName);
          console.log(`🗑️ Removed tag from printer: ${printerName}`, fullTag);
        }
      };
    });
  });
...
function updateAllJobMatches() {
  ...
  jobDataMap.forEach((job, jobID) => {
    ...
    const newMatches = getMatchingPrintersForTags(job.tags);
...
function updateDefaultPrinterTagsUI(container, tagList) {
  ...
  tagList.forEach((tag, index) => {
    if (index < 2 || totalTags <= 3) {
      ...
      inner.textContent = tag.split(":")[1] || tag;
    ...
    printers.forEach(printer => {
      ...
      for (let i = 0; i < quantity; i++) {
        ...
        if (tagContainer) {
          ...
          const inner = document.createElement("div");
          inner.className = "demo-tag-text printer";
          inner.textContent = fullPrinterName;
...
function addTagToFile(fileName, tag) {
  const fileData = fileDataMap.get(fileName);
  if (!fileData) return;
  ...
  if (!fileData.defaultTags.includes(tag)) {
    ...
    fileDataMap.set(fileName, structuredClone(fileData));
    updateAllInstancesOfFile(fileName);
    console.log("➕ Added tag to file:", tag, fileData.defaultTags);
  }
}

function removeTagFromFile(fileName, tagValueOnly) {
  const fileData = fileDataMap.get(fileName);
  if (!fileData || !Array.isArray(fileData.defaultTags)) return;

  fileData.defaultTags = fileData.defaultTags.filter(t => t.split(":")[1] !== tagValueOnly);
  fileDataMap.set(fileName, structuredClone(fileData));
  updateAllInstancesOfFile(fileName);
  console.log("🗑️ Removed tag from file:", tagValueOnly, fileData.defaultTags);
}
...
function initFileTagListeners() {
  ...
  document.querySelectorAll('.add-tag.file').forEach(addTagBtn => {
    const fileTab = addTagBtn.closest('.file-info-tab');
    const fileName = fileTab?.querySelector('.file-text.sidebar.filename')?.textContent.trim();
    if (!fileName || !fileDataMap.has(fileName)) {
      console.log("FILE TAG ISSUE", fileName, fileDataMap);
      return;
    }

    const fileData = fileDataMap.get(fileName);
    const modal = fileTab.querySelector('.add-tag-modal');
    const modalBg = fileTab.querySelector('.tag-modal-background');
    const currentTagsWrap = modal.querySelector('.current-tags');
    const tagListWrap = modal.querySelector('.tag-list-wrap');
    const fileTagsWrap = fileTab.querySelector('.default-file-tags');

    // Clean current modal tags
    currentTagsWrap.querySelectorAll(".demo-tag.in-modal:not([style*='display: none'])").forEach(tag => tag.remove());
    fileData.defaultTags.forEach(tag => {
      const displayValue = tag.includes(":") ? tag.split(":")[1] : tag;
      addTagToModal(displayValue, currentTagsWrap);
    });
    ...
    addTagBtn.addEventListener("click", e => {
      ...
      // ✅ Dynamically populate tags from globalTagMap
      populateTagListWrap(tagListWrap, fileData.defaultTags);
      ...
      tagListWrap.querySelectorAll('.tag-name-in-list').forEach(tagEl => {
        ...
        tagWrap.style.display = fileData.defaultTags.includes(fullTag) ? 'none' : 'flex';

        tagEl.onclick = () => {
          addTagToFile(fileName, fullTag);
          addTagToModal(tagValue, currentTagsWrap);
          updateDefaultTagsUI(fileTagsWrap, fileData.defaultTags);
          if (tagWrap) tagWrap.style.display = 'none';
        };
...
const tagType = getTagType(tagValueOnly);
const fullTag = `${tagType}:${tagValueOnly}`;
removeTagFromFile(fileName, tagValueOnly);
...
function updateTabWithID(tabID, fileName) {
  ...
  console.log("tabFile", tabFile, "tabID", tabID);
...
function populateTagListWrap(tagListWrap, currentTags = []) {
  tagListWrap.innerHTML = ''; // Clear old tags

  for (const [type, tagSet] of Object.entries(globalTagMap)) {
    Array.from(tagSet).sort().forEach(tagValue => {
      const fullTag = `${type}:${tagValue}`;
      const isAlreadyUsed = currentTags.some(tag => {
        if (tag.includes(":")) {
          return tag.split(":")[1] === tagValue;
        } else {
          return tag === tagValue;
        }
      });
      ...
      wrap.className = "tag-in-list-wrap";
      wrap.setAttribute("data-type", type);
      wrap.setAttribute("data-value", tagValue);

      const tag = document.createElement("div");
      tag.className = "tag-name-in-list";
      tag.textContent = tagValue;
      tag.dataset.tagType = type; // ✅ Fix

      wrap.appendChild(tag);
      tagListWrap.appendChild(wrap);
    });
  }
}

function getTagType(tagValue) {
  for (const [type, set] of Object.entries(globalTagMap)) {
    if (set.has(tagValue)) return type;
  }
  return null;
}
