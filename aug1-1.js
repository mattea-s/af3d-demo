...
function initPrinterTagListeners() {
  ...
  // ✅ Add new tag if there's no match AND it's not already known
  if (filter && !matchFound && !tagRepoMap.has(filter)) {
    console.log(`Creating new tag: "${filter}"`);

    // Add to global tag list
    tagRepoMap.set(filter, 'custom');

    // Add to current printer
    printer.printerTags.push(filter);
    printer.printerTagsNum = printer.printerTags.length;
    printerDataMap.set(printerName, structuredClone(printer));

    // Add to modal immediately
    addTagToModal(filter, currentTagsWrap);
    updateDefaultPrinterTagsUI(printerCard.querySelector('.default-file-tags'), printer.printerTags);

    // Add it to tag list UI for consistency
    const tagWrap = document.createElement("div");
    tagWrap.className = "tag-in-list-wrap";
    tagWrap.style.display = "none"; // will re-show via search match
    tagListWrap.appendChild(tagWrap);
    ...
    // Clear input and re-trigger search
    this.value = '';
