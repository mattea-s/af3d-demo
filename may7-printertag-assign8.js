...
function renderQueuedJob(job) {
  ...
  const existingDiv = document.querySelector(`.w-tab-pane[data-w-tab="Queued"] [data-job-id="${jobID}"]`);
  if (existingDiv) {
    console.warn(`⚠️ Job ${jobID} already exists in Queued. Skipping append.`);
    return;
  }
