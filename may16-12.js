...
function updateAllInstancesOfFile(fileName) {
  ...
  updateTabWithID(fileName); // All Files
  updateTabWithID(`fav-${fileName}`);
  updateTabWithID(`flag-${fileName}`);
}


function updateTabWithID(tabID) {
  ...
  console.log(tabFile);
  if (tabID === tabFile) {
    const tab = document.querySelector(`.file-tab[data-w-tab="${tabID}"]`);
    const pane = document.querySelector(`.file-info-tab[data-w-tab="${tabID}"]`);
  } else {
    const tab = document.querySelector(`.file-tab[data-tab-id="${tabID}"]`);
    const pane = document.querySelector(`.file-info-tab[data-tab-id="${tabID}"]`);
  }
  console.log(tab);
  console.log(pane);
  ...
  if (flagCheck) {
    console.log(fileData.fileFlag);
    if (fileData.fileFlag) flagCheck.checked = true;
    else flagCheck.checked = false;
    console.log(flagCheck.checked);
  }

  // Update pane UI (inputs, notes, tags)
  pane.querySelector(".sidebar input.filament-weight").value = fileData.filamentWeight;
  pane.querySelector(".sidebar input.release-temp").value = fileData.defaultReleaseTemp;
  pane.querySelector(".sidebar textarea.production-notes").value = fileData.productionNotes;

  const tagContainer = pane.querySelector(".default-file-tags");
  updateDefaultTagsUI(fileData.defaultTags, tagContainer);
}
