...
const tagColorMap = new Map(); // Map<tagName, colorClass>, e.g., ("Ender 3", "red-1")
...
function applyColorClass(el, colorClass) {
  const known = getAllKnownColorClasses();
  el.classList.forEach(c => { if (known.has(c)) el.classList.remove(c); });
...
function updateDefaultTagsUI(container, tagList) {
  ...
  tagList.forEach((tag, index) => {
    ...
    if (index < 2 || totalTags <= 3) {
      ...
      if (isColourCategory && String(tag).toLowerCase() === "white") {
        inner.classList.add("printer");
      }
...
if (!tagSearch.dataset.keyListenerAttached) {
  tagSearch.addEventListener('keydown', function (e) {
    ...
    if (!tagAlreadyExists && !matchesVisible) {
      // 1. Add to tagRepoMap
      tagRepoMap.set(rawInput, 'custom');
      ...
      // 3. Add to tag list
      const { tagWrap } = createTagElement({ tag:rawInput, category: 'custom', container: tagListWrap });
  ...
  tagSearch.dataset.keyListenerAttached = "true";
      ...
      tagRepoMap.forEach((category, name) => {
        ...
        const { tagWrap } = createTagElement({ tag: name, category: 'custom', container: tagListWrap });
  ...
  if (e.target.closest('.edit-tag-modal')) return;
      ...
      currentTagsWrap.onclick = (e) => {
        ...
        if (!restoreWrap) {
          ...
          const category = tagRepoMap.get(tag) || 'custom';
  ...
  document.querySelectorAll('.add-tag.printer').forEach(addTagBtn => {
    ...
    addTagBtn.addEventListener('click', e => {
      ...
      tagRepoMap.forEach((category, name) => {
        ...
        const { tagWrap } = createTagElement({ tag, category: 'custom', container: tagListWrap });
...
function updateDefaultPrinterTagsUI(container, tagList) {
  ...
  tagList.forEach((tag, index) => {
    ...
    if (index < 2 || totalTags <= 3) {
      ...
      const normCat = (category || "").toString().trim().toLowerCase();
      const isColourCategory = normCat === "color" || normCat === "colour";
      const savedColor = tagColorMap && tagColorMap.get(tag);
      const fallbackColor = isColourCategory ? String(tag || "").toLowerCase() : null;
      const colorToUse = savedColor || fallbackColor;
