  ...
  for (const job of jobs) {
    ...
    if (job.distribute) {
      ...
      for (let i = 0; i < assignCount; i++) {
        ...
        updateJobStatus(job.jobID, "p");
        console.log("Job status updated")
        promoteOrUpdateJob(job.jobID);
...
function jobToProduction(jobID, jobDiv) {
  ...
  console.log("Printing popup in Production:", printingPopup);
  const printerTextTemplate = printingPopup.querySelector(".matching-printers-text");
  if (!printerTextTemplate) {
  console.warn(`❌ No printerTextTemplate found inside #in-production-job-template`);
}
  printingPopup.querySelectorAll(".matching-printers-text").forEach(el => el.remove());
  
  if (job.activePrinterNames.length > 0) {
  ...
  cloneProd.querySelector(".demo-progress.done").style.width = ((job.jobProgCol + job.jobProgDone) / job.quantity * 100) + '%';
  cloneProd.querySelector(".demo-progress.printing").style.width = (job.jobProgProd / job.quantity * 100) + '%';
...
function updateJobInProduction(jobID, jobDiv) {
  ...
  const templateEl = printingPopup.querySelector(".matching-printers-text");
  if (!templateEl) {
    console.warn(`❌ No printerTextTemplate found inside #in-production-job-template`);
    return; // 🔐 Prevent crash from trying to clone null
  }

  const printerTextTemplate = templateEl.cloneNode(true); // ✅ clone BEFORE removing
  const container = printingPopup.querySelector(".div-block-453") || printingPopup;

  // 🧹 Clear previous printer list
  printingPopup.querySelectorAll(".matching-printers-text").forEach(el => el.remove());
  ...
  // 🖱️ Show popup on hover if applicable
  const hoverTarget = jobDiv.querySelector(".file-text.jobs.ul");
  console.log("Hover Target in Production:", hoverTarget);
  ...
  jobDiv.querySelector(".demo-progress.done").style.width = ((job.jobProgCol + job.jobProgDone) / job.quantity * 100) + '%';
  jobDiv.querySelector(".demo-progress.printing").style.width = (job.jobProgProd / job.quantity * 100) + '%';

  prodBackground.appendChild(jobDiv);
}
