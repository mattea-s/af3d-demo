...
function initializeFileData(limit = 10) {
  const tabContainer = document.querySelector(".files-tabs-wrap.w-tab-menu"); // adjust as needed
  const paneContainer = document.querySelector(".file-info-content.w-tab-content");     // adjust as needed
  ...
  repoEntries.forEach(([fileName, repoData], index) => {
    ...
    if (fileWeightInput) fileWeightInput.textContent = repoData.filamentWeight;
    if (fileTempInput) fileTempInput.textContent = repoData.defaultReleaseTemp;
    if (fileNotesText) fileNotesText.textContent = repoData.productionNotes;
  ...
  Webflow.require('tabs').init();

  console.log("✅ Files Initialized:", fileDataMap);

  // 🔁 Hook up modals + editable fields
...
function bindDynamicTabClicks() {
  console.log("🔁 Binding dynamic file tab clicks...");
  const allTabs = document.querySelectorAll('.file-tab');
  const allPanes = document.querySelectorAll('.w-tab-pane');

  allTabs.forEach(tab => {
    console.log("🧩 Found tab:", tab.getAttribute("data-w-tab"));
    tab.addEventListener('click', function () {
      console.log("➡️ Tab clicked:", tab.getAttribute("data-w-tab"));
      ...
      const targetPane = document.querySelector(`.w-tab-pane[data-w-tab="${target}"]`);
      if (targetPane) {
        targetPane.classList.add('w--tab-active');
        console.log("✅ Activated pane for:", tab.getAttribute("data-w-tab"));
      } else {
        console.warn("⚠️ No pane found for tab:", tab.getAttribute("data-w-tab"));
      }
    });
  });
}
