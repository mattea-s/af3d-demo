...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".demo-button.file-save").forEach(saveBtn => {
    saveBtn.addEventListener("click", () => {
      const fileTab = saveBtn.closest(".file-tab");
      const fileName = fileTab?.dataset.filename;
      if (!fileName || !fileDataMap.has(fileName)) return;

      const fileData = fileDataMap.get(fileName);

      // 🏷️ Tags
      const tagEls = fileTab.querySelectorAll(".default-file-tags .demo-tag-text");
      fileData.defaultTags = Array.from(tagEls).map(el => el.textContent.trim());
      fileData.defaultTagsNum = fileData.defaultTags.length;

      // ⚖️ Weight
      const weightInput = fileTab.querySelector(".filament-weight-wrap .file-text.sidebar.filament-weight");
      const parsedWeight = parseFloat(weightInput?.textContent.trim());
      if (!isNaN(parsedWeight)) fileData.filamentWeight = parsedWeight;

      // 🌡️ Release Temp
      const tempInput = fileTab.querySelector(".file-temp-input");
      const newTemp = parseInt(tempInput?.value);
      if (!isNaN(newTemp)) fileData.defaultReleaseTemp = newTemp;

      // 📝 Notes
      const notesInput = fileTab.querySelector(".production-notes");
      if (notesInput) fileData.productionNotes = notesInput.value;

      // 🚩 Flag checkbox
      const flagCheckbox = fileTab.querySelector(".file-flag .flag-check");
      fileData.fileFlag = flagCheckbox?.checked || false;

      // 🔁 Update data map
      fileDataMap.set(fileName, structuredClone(fileData));
      console.log(`✅ File "${fileName}" saved:`, fileData);
    });
  });
});
document.addEventListener("click", e => {
  const isMinus = e.target.closest(".div-block-438.qtyminus");
  const isPlus = e.target.closest(".div-block-438.qtyplus");
  const wrap = e.target.closest(".file-tab");

  if (!wrap || (!isMinus && !isPlus)) return;

  const input = wrap.querySelector(".file-temp-input");
  let value = parseInt(input?.value) || 29;

  if (isPlus) value++;
  if (isMinus && value > 0) value--;

  input.value = value;
});

function initTagListenersForFiles() {
  document.querySelectorAll(".file-tab .add-tag").forEach(addTagBtn => {
    const fileTab = addTagBtn.closest(".file-tab");
    const fileName = fileTab?.dataset.filename;
    if (!fileName || !fileDataMap.has(fileName)) return;
    ...
    addTagBtn.addEventListener('click', () => {
      ...
      updateDefaultTagsUI(fileTab.querySelector('.default-file-tags'), fileData.defaultTags);
      ...
      currentTagsWrap.onclick = (e) => {
        ...
        const idx = fileData.defaultTags.indexOf(tag);
        if (idx !== -1) {
          fileData.defaultTags.splice(idx, 1);
          fileData.defaultTagsNum = fileData.defaultTags.length;
          removeTagFromModal(tag, currentTagsWrap);
          updateDefaultTagsUI(fileTab.querySelector('.default-file-tags'), fileData.defaultTags);
