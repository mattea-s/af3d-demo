...
function initPrinterTagListeners() {
  ...
  let tagType = getTagType(tagValueOnly);
  let fullTag = tagType ? `${tagType}:${tagValueOnly}` : null;

  console.log("🟡 Clicked tag:", tagValueOnly);
  console.log("🟡 Tag type from globalTagMap:", tagType);
  console.log("🟡 Initial fullTag candidate:", fullTag);
  console.log("🟡 Current printer.printerTags:", printer.printerTags);

  // Fallback search if tagType failed or fullTag not in printer.printerTags
  if (!fullTag || !printer.printerTags.includes(fullTag)) {
    const fallback = printer.printerTags.find(t => t.endsWith(`:${tagValueOnly}`));
    console.warn("🔍 Using fallback fullTag:", fallback);
    if (fallback) fullTag = fallback;
  }

  if (!fullTag || !printer.printerTags.includes(fullTag)) {
    console.error("❌ FULL TAG NOT FOUND. Skipping removal.", { fullTag, tagValueOnly });
    return;
  }
  ...
  console.log("✅ Removed from printer.printerTags:", fullTag);
  console.log("📦 Updated printer.printerTags now:", printer.printerTags);

  // 🧼 Remove visually from modal
  removeTagFromModal(clicked, fullTag, currentTagsWrap);

  // 🔄 Update card display
  const defaultTagsContainer = printerCard.querySelector('.default-file-tags');
  updateDefaultPrinterTagsUI(defaultTagsContainer, printer.printerTags);

  // ♻️ Restore tag in selection list if available
  const restoreWrap = Array.from(tagListWrap.querySelectorAll('.tag-in-list-wrap')).find(w =>
    w.querySelector('.tag-name-in-list')?.textContent.trim() === tagValueOnly
  );
  if (restoreWrap) {
    console.log("♻️ Restoring tag to selection list:", tagValueOnly);
    restoreWrap.style.display = 'flex';
  }

  updateAllJobMatches();
  updatePrinterCompatibleJobs(printerName);
  console.log("🔁 Done syncing job matches and compatible printers for:", printerName);
};
    });
  });
}
