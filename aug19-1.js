    ...
    printers.forEach(printer => {
      ...
      for (let i = 0; i < quantity; i++) {
        ...
        if (tagContainer) {
          ...
          tagDiv.textContent = baseModelName;
...
// Helpers
function getAllKnownColorClasses() {
  const set = new Set();
  document.querySelectorAll(".tag-colour-option").forEach(opt => {
    opt.classList.forEach(c => { if (c !== "tag-colour-option") set.add(c); });
  });
  return set;
}
function applyColorClass(el, color) {
  ...
  // Keep only structural classes
  const keepClasses = ['demo-tag', 'tag-name-in-list'];
...
function applyStoredColor(el, tagName) {
  const color = tagColorMap.get(tagName) || "";
  if (color) applyColorClass(el, color);
}
function updateAllTagInstances(oldName, { newName, color }) {
  // Hit *any* element that carries the canonical tag attribute
  const sel = `[data-tag-name="${CSS.escape(oldName)}"]`;
  document.querySelectorAll(sel).forEach(el => {
    // Case A: list row text nodes (the label itself carries data-tag-name)
    if (el.classList.contains('tag-name-in-list')) {
      if (newName) {
        el.textContent = newName;
        el.dataset.tagName = newName;
      }
      if (color !== undefined) applyColorClass(el, color);
      return;
    }

    // Case B: chips (container carries data-tag-name, text is inside .demo-tag-text)
    if (el.classList.contains('demo-tag')) {
      const textEl = el.querySelector('.demo-tag-text');
      if (newName && textEl) {
        textEl.textContent = newName;
        el.dataset.tagName = newName; // keep the attribute in sync on the container
      }
      if (color !== undefined) applyColorClass(el, color);
      return;
    }

    // Fallback: any other element we decide to mark in future
    if (newName) el.dataset.tagName = newName;
    if (color !== undefined) applyColorClass(el, color);
  });
}
...
function updateDefaultTagsUI(container, tagList) {
  ...
  tagList.forEach((tag, index) => {
    ...
    if (index < 2 || totalTags <= 3) {
      ...
      div.dataset.tagName = tag;
      div.textContent = tag;

      const normalizedCat = (category || "").trim().toLowerCase();
      const isColourCategory = normalizedCat === "color" || normalizedCat === "colour";

      const savedColor = tagColorMap && tagColorMap.get(tag);
      const fallbackColor = isColourCategory ? String(tag || "").toLowerCase() : null;
      const colorToUse = savedColor || fallbackColor;

      if (colorToUse) {
        applyColorClass(div, colorToUse);
      }
...
function addTagToModal(tag, container) {
  ...
  const savedColor = tagColorMap && tagColorMap.get(tag);
  if (savedColor) div.classList.add(savedColor);
...
if (!tagSearch.dataset.listenerAttached) {
  tagSearch.addEventListener('input', function () {
    const rawInput = this.value.trim();
    const filter = rawInput.toLowerCase();
    const items = tagListWrap.querySelectorAll('.tag-in-list-wrap');

    items.forEach(item => {
      if (item.id && item.id.includes('template')) return;

      const itemsText = item.querySelector('.tag-name-in-list');
      if (!itemsText) return;

      const text = itemsText.textContent.toLowerCase();
      item.style.display = text.includes(filter) ? '' : 'none';
    });
  });
  tagSearch.dataset.listenerAttached = "true";
}

if (!tagSearch.dataset.keyListenerAttached) {
  tagSearch.addEventListener('keydown', function (e) {
    ...
    const tagAlreadyExists = tagRepoMap.has(filter);
    const visibleItems = tagListWrap.querySelectorAll('.tag-in-list-wrap');
    const matchesVisible = Array.from(visibleItems).some(item => {
      const text = item.querySelector('.tag-name-in-list')?.textContent.toLowerCase();
      return text && text.includes(filter);
    });

    if (!tagAlreadyExists && !matchesVisible) {
      // 1. Add to tagRepoMap
      tagRepoMap.set(rawInput, '');

      // 2. Add to job
      addTagToJob(jobID, rawInput);
      addTagToModal(rawInput, currentTagsWrap);
      ...
      // 3. Add to tag list
      const { tagWrap } = createTagElement({ tag:rawInput, category: '', container: tagListWrap });
      ...
      // 4. Clear input and re-trigger filter
      this.value = '';
      this.dispatchEvent(new Event('input'));
    }
  });
  tagSearch.dataset.keyListenerAttached = "true";
}
      ...
      tagRepoMap.forEach((category, name) => {
        ...
        const { tagWrap } = createTagElement({ tag: name, category: '', container: tagListWrap });

        tagWrap.onclick = (e) => {
  if (e.target.closest('.edit-tag-modal')) return;
      ...
      currentTagsWrap.onclick = (e) => {
        ...
        if (!restoreWrap) {
          ...
          const category = tagRepoMap.get(tag) || '';
          const { tagWrap } = createTagElement({ tag, category, container: tagListWrap });
...
function initPrinterTagListeners() {
  ...
  document.querySelectorAll('.add-tag.printer').forEach(addTagBtn => {
    ...
    addTagBtn.addEventListener('click', e => {
      ...
      tagRepoMap.forEach((category, name) => {
        const tag = name;
        if (printer.printerTags.includes(tag) || category === "printer") return;
      ...
      if (!tagSearch.dataset.keyListenerAttached) {
       tagSearch.addEventListener('keydown', function (e) {
          ...
          if (!tagAlreadyExists && !matchesVisible) {
            // 1. Add to global tag repo
            tagRepoMap.set(rawInput, '');

            // 2. Add to current printer
            printer.printerTags.push(rawInput);
            printer.printerTagsNum = printer.printerTags.length;
            printerDataMap.set(printerName, structuredClone(printer));

            // 3. Add to modal visually
            addTagToModal(rawInput, currentTagsWrap);
            updateDefaultPrinterTagsUI(printerCard.querySelector('.default-file-tags'), printer.printerTags);

            // 4. Add to the tag list UI
            const { tagWrap } = createTagElement({ tag: rawInput, category: '', container: tagListWrap });

            tagWrap.onclick = (e) => {
                if (e.target.closest('.edit-tag-modal')) return;
              if (!printer.printerTags.includes(rawInput)) {
                printer.printerTags.push(rawInput);
                printer.printerTagsNum = printer.printerTags.length;
                printerDataMap.set(printerName, structuredClone(printer));
            ...
            // 5. Clear input and re-trigger filter
            this.value = '';
            this.dispatchEvent(new Event('input'));
...
function updateDefaultPrinterTagsUI(container, tagList) {
  ...
  tagList.forEach((tag, index) => {
    const category = tagRepoMap.get(tag);
    console.log(tagColorMap.get(tag));
    ...
    if (index < 2 || totalTags <= 3) {
      ...
      if (savedColor !== " " && savedColor !== "") {
        applyColorClass(div, savedColor);
        div.classList.remove("printer");
...
if (tagSearch) {
  ...
  if (!tagSearch.dataset.keyListenerAttached) {
    tagSearch.addEventListener('keydown', function (e) {
      ...
      const tagAlreadyExists = tagRepoMap.has(filter);
      const matchesVisible = Array.from(tagListWrap.querySelectorAll('.tag-in-list-wrap')).some(item => {
        const text = item.querySelector('.tag-name-in-list')?.textContent.toLowerCase().trim();
        return text === filter;
      });
      ...
      if (!tagAlreadyExists && !matchesVisible) {
        ...
        // 2. Add to file
        fileData.defaultTags.push(rawInput);
        ...
        tagWrap.onclick = (e) => {
            if (e.target.closest('.edit-tag-modal')) return;
          if (!fileData.defaultTags.includes(rawInput)) {
            fileData.defaultTags.push(rawInput);
            fileData.defaultTagsNum = fileData.defaultTags.length;
            fileDataMap.set(fileName, structuredClone(fileData));
        ...
        // 5. Clear input and trigger filter
        this.value = '';
        this.dispatchEvent(new Event('input'));
      }
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('printer-search').addEventListener('input', function () {
  const query = this.value.toLowerCase();
  const itemsContainer = document.querySelector('.demo-printer-card-area');
  const items = itemsContainer.querySelectorAll('.demo-printer-card');

  items.forEach(item => {
    if (item.id.includes('template')) return;
    const itemText = item.querySelector('.demo-printer-name');
  ...
  const query = this.value.toLowerCase();
  const itemsContainer = document.querySelector('.files-tab-pane.w-tab-pane.w--tab-active');
  const items = itemsContainer.querySelectorAll('.file-tab.div-block-430');

  items.forEach(item => {
    if (item.id.includes('template')) return;
    const itemText = item.querySelector('.file-text.filename');
    const text = itemText.textContent.toLowerCase();
    if (text.includes(query)) {
      item.style.display = '';
    } else {
  ...
  const delTagModal = document.querySelector('.file-send-modal.tag-delete');
delTagModal.addEventListener("click", (e) => {
  delTagModal.style.display = "none";
});

  const observer = new MutationObserver(() => {
  const container = document.querySelector('.q-send-job-wrap');
  container.querySelectorAll('.add-tag-modal').forEach(popup => {
    const isVisible = getComputedStyle(popup).display !== 'none';
    if (!isVisible) return;

    const item = popup.closest('.default-file-tags');
    const btn = item?.querySelector('.add-tag');

    if (!btn || !item || !popup || !container) return;
...
// Observe changes to the body or list container
observer.observe(document.body, {
  ...
  const FLIP_MARGIN = 8; // px from viewport edge

function positionEditModal(popup) {
  if (!popup) return;
  ...
  popup.style.bottom = 'auto';
  popup.style.left = '102%';
  popup.style.right = 'auto';

  // Measure and flip if it would overflow on the right
  const rect = popup.getBoundingClientRect();
  if (rect.right > (window.innerWidth - FLIP_MARGIN)) {
    popup.style.left = 'auto';
    popup.style.right = '102%';
    popup.style.top = '-200px';
  }
}

// If you already have an "open edit modal" event, call positionEditModal(popup) there.
// If not, this minimal observer will do it without causing style loops:
const observer = new MutationObserver(() => {
  document.querySelectorAll('.edit-tag-modal').forEach(positionEditModal);
});
observer.observe(document.body, { subtree: true, childList: true }); // no attributes to avoid feedback loops
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("delete-tag-btn").addEventListener("click", () => {
    if (!tagToDelete) {
      console.log("No tag to delete found");
      return; // ⬅️ Important: prevent continuing if null
    }

    const tag = tagToDelete;

    // ✅ Step 4: Delete from repo
    tagRepoMap.delete(tag);
    ...
    // Remove from printers
    printerDataMap.forEach((printer, printerName) => {
      printer.printerTags = printer.printerTags.filter(t => t !== tag);
      printer.printerTagsNum = printer.printerTags.length;
      printerDataMap.set(printerName, structuredClone(printer));
    });

    // Remove from files
    fileDataMap.forEach((file, fileName) => {
      file.defaultTags = file.defaultTags.filter(t => t !== tag);
      file.defaultTagsNum = file.defaultTags.length;
      fileDataMap.set(fileName, structuredClone(file));
    });

    // Update UIs
    updateAllInstancesOfFile();
    updateAllJobMatches();

    document.querySelectorAll(".default-file-tags").forEach(container => {
      const fileName = container.closest(".file-info-wrap")?.dataset.fileId;
      if (fileName && fileDataMap.has(fileName)) {
        updateDefaultTagsUI(container, fileDataMap.get(fileName).defaultTags);
      }
    });

    document.querySelectorAll(".demo-printer-card").forEach(card => {
      const name = card.querySelector(".demo-printer-name")?.textContent.trim();
      if (name && printerDataMap.has(name)) {
        updateDefaultPrinterTagsUI(card.querySelector('.default-file-tags'), printerDataMap.get(name).printerTags);
      }
    });

    document.querySelectorAll(".demo-q-file").forEach(fileBlock => {
      const jobID = fileBlock.dataset.jobId;
      if (jobID && jobDataMap.has(jobID)) {
        updateDefaultTagsUI(fileBlock.querySelector('.default-file-tags.tags'), jobDataMap.get(jobID).tags);
      }
    });

    // Remove from tag list in modals
    document.querySelectorAll('.tag-in-list-wrap').forEach(wrap => {
      const text = wrap.querySelector('.tag-name-in-list')?.textContent.trim();
      if (text === tag) wrap.remove();
    });

    // ✅ Step 5: close and clear
    document.getElementById("delete-tags-modal").style.display = "none";
    tagToDelete = null;
  });

  // ✅ Cancel button click
  document.getElementById("cancel-delete-tag-btn").addEventListener("click", () => {
    document.getElementById("delete-tags-modal").style.display = "none";
    tagToDelete = null;
  });

  // ✅ Background click
  document.querySelector(".tag-modal-background")?.addEventListener("click", () => {
    document.getElementById("delete-tags-modal").style.display = "none";
    tagToDelete = null;
  });
});
...
function normalizeCategory(cat) {
  console.log("normalizeCategory", cat);
  if (!cat) return "";
  const lower = cat.toLowerCase();
  if (["color", "colour"].includes(lower)) return "Colour";
  return cat;
}

function isKnownColorTag(tag) {
  console.log("isKnownColorTag", tag);
  const knownColors = ["red", "blue", "green", "yellow", "black", "white", "grey", "orange", "purple"];
  return knownColors.includes(tag.trim().toLowerCase());
}

function createTagElement({ tag, category, container }) {
  console.log("createTagElement", tag, category, container);
  const tagWrap = document.createElement("div");
  tagWrap.className = "tag-in-list-wrap";
  tagWrap.style.display = "flex";
  container.prepend(tagWrap);

  const tagEl = document.createElement("div");
  tagEl.className = "tag-name-in-list";
  tagEl.textContent = tag;
  tagEl.dataset.tagName = tag; // ADD: used for global updates
  ...
  applyStoredColor(tagEl, tag); // ADD: apply saved colour (if any)

  const tagEditWrap = document.createElement("div");
  tagEditWrap.className = "div-block-456";
  tagWrap.appendChild(tagEditWrap);

  const editModalTemplate = document.getElementById("edit-tag-modal-template");
  let editModal = null;

  if (editModalTemplate) {
    const editModalClone = editModalTemplate.cloneNode(true);
    editModalClone.id = "";
    editModalClone.style.display = "none";
    tagWrap.appendChild(editModalClone);
    editModal = tagWrap.querySelector(".edit-tag-modal");
  }
  ...
  tagEdit.addEventListener("click", (e) => {
    ...
    const tagPreviewEl = editModal.querySelector('.tag-preview');
    const tagPreviewText = editModal.querySelector('.tag-preview-text');
    tagPreviewText.textContent = originalName;

    const nameInput = editModal.querySelector('.tag-name-wrap .tag-edit-input');
    if (nameInput) nameInput.value = originalName;
    nameInput.addEventListener("change", () => {
      tagPreviewText.textContent = nameInput.value;
    });

    const categoryInput = editModal.querySelector('.tag-category-wrap .tag-edit-input');
    if (categoryInput) {
      let category = tagRepoMap.get(originalName) || "";
      if (!category && isKnownColorTag(originalName)) category = "Colour";
      category = normalizeCategory(category);
      categoryInput.value = category;
    }

    // ADD: track current/original colour via sidecar map
    const originalColor = tagColorMap.get(originalName) || "";
    if (tagPreviewEl) applyColorClass(tagPreviewEl, originalColor);

    // ADD: pending colour for this session (preview until Save)
    let pendingColor = originalColor;

    const colorOptions = editModal.querySelectorAll(".tag-colour-option");
    colorOptions.forEach(option => {
      option.addEventListener("click", () => {
        const selectedColor = [...option.classList].find(cls => cls !== "tag-colour-option");
        if (selectedColor) {
          pendingColor = selectedColor;                // remember choice
          applyColorClass(tagEl, selectedColor);       // live preview on element
          if (tagPreviewEl) applyColorClass(tagPreviewEl, selectedColor); // preview chip
        }
      });
    });

    const originalClass = [...tagEl.classList].find(cls => cls !== "tag-name-in-list");

    const saveBtn = editModal.querySelector('.tag-edit-save-text:not(.cancel)');
    if (saveBtn) {
      saveBtn.onclick = () => {
        const oldName = originalName;
        const newName = (nameInput?.value || "").trim() || oldName;
        const newCategory = normalizeCategory((categoryInput?.value || "").trim());

        // KEEP your category repo exactly as-is:
        if (newName !== oldName) {
          // move category entry if renamed
          const oldCat = tagRepoMap.get(oldName);
          if (oldCat !== undefined) tagRepoMap.delete(oldName);
          tagRepoMap.set(newName, newCategory);
        } else {
          tagRepoMap.set(newName, newCategory);
        }

        // ADD: persist colour in sidecar map (and move it on rename)
        if (newName !== oldName) {
          tagColorMap.delete(oldName);
        }
        tagColorMap.set(newName, pendingColor || "");

        // Update this element
        tagEl.textContent = newName;
        tagEl.dataset.tagName = newName;
        tagEl.className = "tag-name-in-list";
        applyColorClass(tagEl, pendingColor || "");

        // Propagate globally (name + colour)
        updateAllTagInstances(oldName, { newName, color: pendingColor || "" });

        // Keep your preview text behaviour
        const previewText = editModal.querySelector(".tag-preview-text");
        if (previewText) previewText.textContent = newCategory;
        tagPreviewText.textContent = newName;
    ...
    const cancelBtn = editModal.querySelector('.tag-edit-save-text.cancel');
    if (cancelBtn) {
      cancelBtn.onclick = () => {
        tagEl.textContent = originalName;
        tagEl.dataset.tagName = originalName;                // ensure dataset reset
        tagEl.className = "tag-name-in-list";
        if (originalClass) tagEl.classList.add(originalClass);
        applyColorClass(tagEl, originalColor);               // revert colour

        if (categoryInput) categoryInput.value = tagRepoMap.get(originalName) || "";
        const previewText = editModal.querySelector(".tag-preview-text");
        if (previewText) previewText.textContent = normalizeCategory(tagRepoMap.get(originalName) || "");
        if (tagPreviewEl) applyColorClass(tagPreviewEl, originalColor);
    ...
    const background = editModal.closest('.tag-modal-background');
    if (background && !background.dataset.listenerAttached) {
      background.addEventListener("click", () => {
        tagEl.textContent = originalName;
        tagEl.dataset.tagName = originalName;
        tagEl.className = "tag-name-in-list";
        if (originalClass) tagEl.classList.add(originalClass);
        applyColorClass(tagEl, originalColor);
      ...
      background.dataset.listenerAttached = "true";
    }
  });
  ...
  tagDel.addEventListener('click', (e) => {
    e.stopPropagation();
    tagToDelete = tag;
    document.getElementById("tag-to-delete-name").textContent = tag;
    document.getElementById("delete-tags-modal").style.display = "flex";
  });
  tagEditWrap.appendChild(tagDel);

  return { tagWrap, tagEl, tagEdit, tagDel, editModal };
}
