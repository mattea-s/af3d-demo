...
tagListWrap.querySelectorAll('.edit-tag-button.delete').forEach(btn => {
  if (btn.dataset.listenerAttached === 'true') return; // prevent duplicates
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Remove from tagRepoMap
  tagRepoMap.delete(tagName);

  // Remove from jobDataMap, printerDataMap, fileDataMap
  ...
  jobDataMap.forEach((job, key) => {
    if (job.tags.includes(tagName)) {
      ...
      updateDefaultTagsUI(document.querySelector(`[data-job-id="${key}"] .default-file-tags.tags`), job.tags);
    }
  });
  ...
  printerDataMap.forEach((printer, key) => {
    if (printer.printerTags.includes(tagName)) {
      ...
      updateDefaultPrinterTagsUI(document.querySelector(`.demo-printer-card:has(.demo-printer-name:contains("${key}")) .default-file-tags`), printer.printerTags);
    }
  });

  // Remove all .tag-in-list-wraps with that name
  document.querySelectorAll('.tag-in-list-wrap').forEach(wrap => {
    const name = wrap.querySelector('.tag-name-in-list')?.textContent.trim();
    if (name === tagName) wrap.remove();
  });
