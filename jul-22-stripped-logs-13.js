...
function initFileCheckboxListeners() {
  ...
  function toggleTabSelection(checkbox, isChecked) {
    ...
    if (isChecked) {
      ...
      console.log(`toggleTabSelection ${fileName} selected`);
      if (!fileSelectionOrder.includes(fileName)) {
        fileSelectionOrder.push(fileName);
      }
    } else {
      tab.classList.remove("selected");
      console.log(`toggleTabSelection ${fileName} removed`);
...
function getMatchingPrintersForTags(fileTags, filamentWeight) {
  ...
  console.log("getMatchingPrintersForTags Matching printers for tags [" + fileTags.join(", ") + "]:", matches);
  return matches;
}
...
function initializeFileData(limit = 20) {
  ...
  repoEntries.forEach(([fileName, repoData], index) => {
    ...
    if (fileFlagCheck) {
      ...
      fileFlagCheck.addEventListener("change", () => {
        ...
        console.log(`initializeFileData [FLAG] ${fileName} flagged = ${fileData.fileFlag}`);
        console.log(`initializeFileData [FLAG] ${fileName} faved = ${fileData.fileFav}`);
        console.log(`initializeFileData [FLAG] ${fileName} tags at flag time:`, fileDataMap.get(fileName)?.defaultTags);
      ...
      if (fileFlagCheck.checked === true) {
        ...
        console.log("initializeFileData Init Files function", fileFlagCheck.checked);
      }
   }
  ...
  if (window.Webflow && Webflow.require) {
    ...
    if (tabs && typeof tabs.ready === 'function') {
      tabs.ready();
      console.log("initializeFileData ✅ Webflow tabs re-initialized for files");
      countFilesInTabs();
    }
  }

  console.log("initializeFileData ✅ Files Initialized:", fileDataMap);
...
function initializeFavoritesTab() {
  ...
  console.log(`initializeFavoritesTab Favourited files: ${favorites}`);
  ...
  favorites.forEach(([fileName, fileData]) => {
    ...
    tab.querySelector(".material-symbols-outlined")?.addEventListener("click", (e) => {
      ...
      console.log(`initializeFavoritesTab ${fileName} fav set to:`, fileData.fileFav);
      console.log(`initializeFavoritesTab ${fileName} flag set to:`, fileData.fileFlag);
    ...
    if (notesArea) {
      ...
      console.log(`initializeFavoritesTab ${notesArea} set to ${fileData.productionNotes}`);
    ...
    if (flagCheck.checked === true) {
      console.log("initializeFavoritesTab Flag checked:", flagCheck.checked);
      tab.classList.add("flagged");
    }
    ...
    flagCheck.addEventListener("change", () => {
      console.log("File flag checked");
      console.log(`File '${fileName}' fav before update:`, fileData.fileFav);
      console.log(`File '${fileName}' flag before update:`, fileData.fileFlag);
      setTimeout(() => {
      console.log(`File '${fileName}' fav after timeout:`, fileData.fileFav);
      console.log(flagCheck, " - Flag changed");
      fileData.fileFlag = flagCheck.checked;
      fileDataMap.set(fileName, structuredClone(fileData));
      console.log(`initializeFavoritesTab File '${fileName}' flag updated to:`, fileData.fileFlag);
      console.log(`initializeFavoritesTab File '${fileName}' fav updated to:`, fileData.fileFav);
...
function initializeFlaggedTab() {
  ...
  console.log("initializeFlaggedTab Flagged files:", flagged.map(([name, data]) => ({
  name,
  ...
  flagged.forEach(([fileName, fileData]) => {
    ...
    tab.querySelector(".material-symbols-outlined")?.addEventListener("click", (e) => {
      ...
      console.log(`initializeFlaggedTab ${fileName} fav set to:`, fileData.fileFav);
      console.log(`initializeFlaggedTab ${fileName} flag set to:`, fileData.fileFlag);
    });
    ...
    if (notesArea) {
      notesArea.value = fileData.productionNotes;
      console.log(`initializeFlaggedTab ${notesArea} set to ${fileData.productionNotes}`);
      notesArea.addEventListener("input", () => {
        fileData.productionNotes = notesArea.value;
        fileDataMap.set(fileName, structuredClone(fileData));
        updateAllInstancesOfFile(fileName);
    ...
    if (flagCheck) flagCheck.checked = fileData.fileFlag ?? false;
    if (flagCheck.checked === true) {
      console.log("initializeFlaggedTab Flagged:", flagCheck.checked);
      tab.classList.add("flagged");
    }
    ...
    flagCheck.addEventListener("change", () => {
      ...
      fileData.fileFlag = flagCheck.checked;
      fileDataMap.set(fileName, structuredClone(fileData));
      console.log(`initializeFlaggedTab File '${fileName}' flag updated to:`, fileData.fileFlag);
      console.log(`initializeFlaggedTab File '${fileName}' fav updated to:`, fileData.fileFav);
...
function initializeAllFilesTab() {
  ...
  allFiles.forEach(([fileName, fileData]) => {
    ...
    if (fileData.fileFav === true) {
      ...
      console.log(`initializeAllFilesTab fav: ${fileData.fileFav}`);
    }
    ...
    if (flagCheck) {
      ...
      console.log(`initializeAllFilesTab flag: ${flagCheck.checked}`);
      flagCheck.addEventListener("change", () => {
        console.log("File flag checked");
        console.log(`File '${fileName}' fav before update:`, fileData.fileFav);
        console.log(`File '${fileName}' flag before update:`, fileData.fileFlag);
...
function updateTabWithID(tabID, fileName) {
  ...
  const fileData = fileDataMap.get(fileName);
  console.log(fileDataMap.get(fileName),fileData, fileDataMap);
  if (!fileData) return;
  ...
  const favIcon = tab.querySelector(".material-symbols-outlined");
  if (favIcon) {
    console.log("File data fav:", fileData.fileFav);
  ...
  .find(el => el.getAttribute("data-w-tab")?.toLowerCase() === folderName.toLowerCase());
      ...
      selectedFiles.forEach(fileObj => {
        ...
        tab.querySelector(".material-symbols-outlined")?.addEventListener("click", (e) => {
          ...
          // Toggle and persist
          console.log(`BEFORE ${fileName} fav set to:`, fileData.fileFav);
          fileData.fileFav = !fileData.fileFav;
          console.log(`1 - ${fileName} fav set to:`, fileData.fileFav);
          fileDataMap.set(fileName, structuredClone(fileData));
          console.log(`2 - ${fileName} fav set to:`, fileData.fileFav);
          ...
          console.log(`LAST ${fileName} fav set to:`, fileData.fileFav);
        });
        ...
        const flagCheck = pane.querySelector("input.flag-check");
        if (flagCheck) flagCheck.checked = fileData.fileFlag ?? false;
        console.log(fileName, " Flagged:", flagCheck.checked);
        if (flagCheck.checked === true) {
          tab.classList.add("flagged");
        ...
        flagCheck.addEventListener("change", () => {
          ...
          console.log(`File '${fileName}' flag before update:`, fileData.fileFlag);
          setTimeout(() => {
            console.log(`File '${fileName}' fav after timeout:`, fileData.fileFlag);
          fileData.fileFlag = flagCheck.checked;
          fileDataMap.set(fileName, structuredClone(fileData));
          console.log(`File '${fileName}' flag updated to:`, fileData.fileFlag);
          console.log(`File '${fileName}' fav updated to:`, fileData.fileFav);
