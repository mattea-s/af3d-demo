...
function populatePrinterCard(card, job, currentIteration, quantity) {
  ...
  const filamentUsage = Math.round(((jobFilamentWeight * 360) / 1000));
  const filamentLeft = Math.round(((printerFilamentRemaining * 360) / 1000) - filamentUsage);
  const filamentUsed = 360 - filamentLeft;

  console.log(filamentUsage + filamentLeft + filamentUsed);

  const spoolBlack = filamentLeft;
  const spoolGrey1 = filamentLeft;
  const spoolGrey2 = filamentUsed;
  const spoolWhite = filamentUsed;
