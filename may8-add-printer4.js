...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function addNewPrinters(printers) {
    ...
    document.querySelectorAll(".demo-printer-card").forEach(card => {
      ...
      console.log(`Printer name: ${fullName}`);
      if (fullName) {
        const baseModel = fullName.split(" - ")[0].trim(); // Strip off the number
        console.log(`Printer model: ${baseModel}`);
        modelCount[baseModel] = (modelCount[baseModel] || 0) + 1;
        console.log(`Printer model count: ${modelCount}`);
      }
    });
    ...
    printers.forEach(printer => {
      ...
      for (let i = 0; i < quantity; i++) {
        ...
        if (tagContainer) {
          const tagDiv = document.createElement("div");
          console.log(`Created new tag: ${tagDiv}`);
          console.log(`Created tag for ${fullPrinterName}`);
        ...
        // ✅ Update modelCount for next printer of same model
