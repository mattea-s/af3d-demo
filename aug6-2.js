...
if (!tagSearch.dataset.keyListenerAttached) {
  ...
  e.stopPropagation(); // Prevent parent tagWrap click from triggering
  const tagName = tagEl.textContent.trim();
  openDeleteTagModal(tagName);
});
...
function openDeleteTagModal(tagName) {
  currentTagToDelete = tagName;
  document.getElementById("tag-to-delete-name").textContent = tagName;
  document.getElementById("delete-tags-modal").style.display = "block";
}
...
document.getElementById("delete-tag-btn").addEventListener("click", () => {
  ...
  const tag = currentTagToDelete;

  // 1. Remove from global tagRepoMap
  tagRepoMap.delete(tag);

  // 2. Remove from jobs
  jobDataMap.forEach((job, jobID) => {
    const newTags = job.tags.filter(t => t !== tag);
    if (newTags.length !== job.tags.length) {
      job.tags = newTags;
      jobDataMap.set(jobID, structuredClone(job));
    }
  });

  // 3. Remove from printers
  printerDataMap.forEach((printer, printerName) => {
    const newTags = printer.printerTags.filter(t => t !== tag);
    if (newTags.length !== printer.printerTags.length) {
      printer.printerTags = newTags;
      printer.printerTagsNum = newTags.length;
      printerDataMap.set(printerName, structuredClone(printer));
    }
  });
  ...
  fileDataMap.forEach((file, fileName) => {
    const newTags = file.defaultTags.filter(t => t !== tag);
    if (newTags.length !== file.defaultTags.length) {
      file.defaultTags = newTags;
      file.defaultTagsNum = newTags.length;
      fileDataMap.set(fileName, structuredClone(file));
    }
  });

  // 5. Update UI globally (you may need to adapt this part depending on your re-render logic)
  document.querySelectorAll(".default-file-tags").forEach(el => {
    const name = el.closest(".file-info-wrap")?.dataset.fileId;
    if (name && fileDataMap.has(name)) {
      updateDefaultTagsUI(el, fileDataMap.get(name).defaultTags);
    }
  });
  ...
  // 6. Remove from visible tag-in-list-wraps
  document.querySelectorAll('.tag-in-list-wrap').forEach(tagWrap => {
    const text = tagWrap.querySelector('.tag-name-in-list')?.textContent.trim();
    if (text === tag) {
      tagWrap.remove();
    }
  });
