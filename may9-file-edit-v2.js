...
function initFileTagListeners() {
  console.log("📁 [initFileTagListeners] Running...");

  document.querySelectorAll('.file-info-wrap').forEach(fileWrap => {
    const fileID = fileWrap.dataset.fileId;
    const fileName = fileWrap.querySelector(".file-text.sidebar")?.textContent.trim();
    if (!fileDataMap.has(fileName)) return;

    const fileData = fileDataMap.get(fileName);
    const addTagBtn = fileWrap.querySelector('.add-tag.file');
    ...
    addTagBtn.addEventListener('click', e => {
      e.preventDefault();
      console.log(`🟢 [${fileName}] Opening tag modal...`);
      modal.style.display = 'block';
      modalBg.style.display = 'block';
      ...
      tagListWrap.querySelectorAll('.tag-name-in-list').forEach(tagEl => {
        ...
        tagEl.onclick = () => {
          if (!fileData.defaultTags.includes(tag)) {
            ...
            console.log(`➕ [${fileName}] Added tag: ${tag}`, fileData.defaultTags);
            tagWrap.style.display = 'none';
          }
        };
      });
      ...
      currentTagsWrap.onclick = (e) => {
        ...
        if (!clicked) return;
        const tag = clicked.textContent.trim();
        const index = fileData.defaultTags.indexOf(tag);
        if (index !== -1) {
          fileData.defaultTags.splice(index, 1);
          fileData.defaultTagsNum = fileData.defaultTags.length;
          removeTagFromModal(tag, currentTagsWrap);
          updateDefaultTagsUI(defaultTagsContainer, fileData.defaultTags);
          fileDataMap.set(fileName, structuredClone(fileData));
          console.log(`🗑️ [${fileName}] Removed tag: ${tag}`, fileData.defaultTags);
...
function initFileEditListeners() {
  console.log("📁 [initFileEditListeners] Running...");
  ...
  document.querySelectorAll('.file-info-wrap').forEach(fileWrap => {
    ...
    const fileData = fileDataMap.get(fileName);
    console.log(`🔍 [${fileName}] Hooking up edit fields`);

    const minusBtn = fileWrap.querySelector('.div-block-438.qtyminus');
    const plusBtn = fileWrap.querySelector('.div-block-438.qtyplus');
    const tempInput = fileWrap.querySelector('.release-temp input');

    if (minusBtn && plusBtn && tempInput) {
      minusBtn.addEventListener('click', () => {
        let current = parseInt(tempInput.value) || 0;
        tempInput.value = Math.max(0, current - 1);
        console.log(`➖ [${fileName}] Temp decreased to ${tempInput.value}`);
      });

      plusBtn.addEventListener('click', () => {
        let current = parseInt(tempInput.value) || 0;
        tempInput.value = current + 1;
        console.log(`➕ [${fileName}] Temp increased to ${tempInput.value}`);
      });

      tempInput.addEventListener('change', () => {
        console.log(`✏️ [${fileName}] Temp manually set to ${tempInput.value}`);
      });
    }

    const saveBtn = fileWrap.querySelector('.demo-button.file-save');
    if (saveBtn) {
      saveBtn.addEventListener('click', () => {
        const newWeight = parseFloat(fileWrap.querySelector('.filament-weight').value) || 0;
        const newTemp = parseInt(tempInput.value) || 29;
        const notes = fileWrap.querySelector('.production-notes')?.value || '';
        const flag = fileWrap.querySelector('.flag-check')?.checked || false;

        fileData.filamentWeight = newWeight;
        fileData.defaultReleaseTemp = newTemp;
        fileData.productionNotes = notes;
        fileData.fileFlag = flag;

        fileDataMap.set(fileName, structuredClone(fileData));

        console.log(`💾 [${fileName}] Saved changes:`);
        console.log(`   🧪 Weight: ${newWeight}`);
        console.log(`   🌡️ Temp: ${newTemp}`);
...
document.addEventListener("DOMContentLoaded", () => {
  console.log("🌐 DOMContentLoaded — Initializing file tag + edit listeners...");
  initFileTagListeners();
  initFileEditListeners();
});
