...
function initializeFileData(limit = 10) {
  ...
  const paneTemplate = document.getElementById("file-tab-pane-template");
  ...
  repoEntries.forEach(([fileName, repoData], index) => {
    ...
    tab.querySelector(".fav-file-embed")?.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
    ...
    if (fileFlagCheck) fileFlagCheck.checked = repoData.fileFlag === true;
...
function initializeFavoritesTab() {
  ...
  favorites.forEach(([fileName, fileData]) => {
    ...
    const fileTabNameText = tab.querySelector(".file-text.filename");
    if (fileTabNameText) fileTabNameText.textContent = fileName;
    tab.querySelector(".fav-file-embed")?.classList.toggle("fav", true);

    tabContainer.appendChild(tab);

    pane.removeAttribute("id");
    pane.style.display = "";
    pane.classList.add("w-tab-pane");
    pane.setAttribute("data-w-tab", fileName);

    // Minimal details; you can extend with full content
    const nameText = pane.querySelector(".file-text.sidebar.filename");
    if (nameText) nameText.textContent = fileName;

    paneContainer.appendChild(pane);
  });

  // Rebind Webflow tab behavior if necessary
  if (window.Webflow && Webflow.require) {
    const tabs = Webflow.require('tabs');
    if (tabs && typeof tabs.ready === 'function') {
      tabs.ready();
...
document.addEventListener("click", function (e) {
  ...
  const fileName = fileTab.getAttribute("data-w-tab");
  const file = fileDataMap.get(fileName);
  if (!file || !e.target.classList.contains("material-symbols-outlined")) return;

  if (e.target.matches(".material-symbols-outlined.fav")) {
    e.target.classList.remove("selected");
    file.fileFav = FALSE;
    fileDataMap.set(fileName, structuredClone(file));
    console.log(`${fileName} removed from favourites`);
  } else if (e.target.matches(".material-symbols-outlined")) {
    e.target.classList.add("selected");
    file.fileFav = TRUE;
    fileDataMap.set(fileName, structuredClone(file));
    console.log(`${fileName} added to favourites`);
  }
  initializeFavoritesTab(); // optional: update tab if needed
});
