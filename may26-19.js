...
function initializePrinterData() {
  ...
  printerCards.forEach((card, index) => {
    ...
    filamentSpool.style.background = "conic-gradient(black " + filamentLeftOnSpool + "deg,white " + filamentLeftOnSpool + "deg)";
...
function populatePrinterCard(card, job, currentIteration, quantity) {
  ...
  const filamentSpool = card.querySelector(".filament-icon.fill");
  const jobFilament = ((job.filamentWeight * 360) / 1000);
  console.log(job.filamentWeight, " = ", jobFilament);
  const filamentLeftOnSpool = (((printerData.filamentRemaining * 360) / 1000) - jobFilament);
  const filamentRemainingCalc = (printerData.filamentRemaining - job.filamentWeight);
  console.log(printerData.filamentRemaining, " - ", job.filamentWeight, " = ", filamentRemainingCalc, " = ", filamentLeftOnSpool);
  const filamentGone = Math.max(0, 360 - filamentLeftOnSpool);
  console.log(filamentGone);

  const blackGrad = filamentGone;
  const greyGrad1 = filamentGone;
  const greyGrad2 = filamentGone - jobFilament;
  const whiteGrad = jobFilament;
  console.log("conic-gradient(black " + blackGrad + "deg,grey " + greyGrad1 + "deg " + greyGrad2 + "deg,white " + whiteGrad + "deg)");

  filamentSpool.style.background = "conic-gradient(black " + blackGrad + "deg,grey " + greyGrad1 + "deg " + greyGrad2 + "deg,white " + whiteGrad + "deg)";

  console.log("Before: ",printerData.filamentRemaining);
  printerData.filamentRemaining -= jobFilament;
