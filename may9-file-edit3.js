...
// 🔁 Listen for Files tab activation and initialize file listeners
document.addEventListener("DOMContentLoaded", () => {
  const filesTabButton = document.querySelector('[data-w-tab="Files"]');

  if (filesTabButton) {
    filesTabButton.addEventListener("click", () => {
      // Wait for tab content to become visible
      setTimeout(() => {
        console.log("📂 Files tab clicked — initializing listeners");
        initTagListenersForFiles();
        initFileSaveListeners();
      }, 50);
    });
  }
});

// ✅ Define this as a separate function (was inline before)
function initFileSaveListeners() {
  document.querySelectorAll(".file-tab .demo-button.file-save").forEach(saveBtn => {
