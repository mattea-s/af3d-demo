...
function initializePrinterData() {
  ...
  printerCards.forEach((card, index) => {
    ...
    filamentMenuOpen.addEventListener("click", () => {
      ...
      filamentModal.style.zIndex = "3000";
      ...
      filamentModal.addEventListener('focusout', (e) => {
        setTimeout(() => {
          // If focus is no longer inside the modal, close and reset it
          if (!filamentModal.contains(document.activeElement)) {
            filamentModal.style.display = "none";
            filamentModal.style.zIndex = "-1";
        ...
        }, 10); // Delay needed because focus hasn't shifted at the time of the event
      });
    ...
    filamentSave.addEventListener("click", () => {
      ...
      const newFilamentLevel = modalFilamentLevel.getAttribute("value").replace(/g$/, '');
