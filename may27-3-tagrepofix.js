...
function addTagToModal(tagValue, container) {
  const exists = Array.from(container.querySelectorAll('.demo-tag.in-modal'))
    .some(el => el.style.display !== 'none' && el.textContent.trim() === tagValue);
  if (exists) return;
...
function removeTagFromModal(tagValueOnly, container) {
  const tags = container.querySelectorAll('.demo-tag.in-modal');

  tags.forEach(tagEl => {
    const tagText = tagEl.textContent.trim();
    if (tagText.toLowerCase().trim() === tagValueOnly.toLowerCase().trim()) {
      tagEl.remove();
    }
  });
}
...
if (printer.printerTags.includes(fullTag)) {
  printer.printerTags = printer.printerTags.filter(t => t !== fullTag);
          ...
          updateAllJobMatches();
          updatePrinterCompatibleJobs(printerName);
          console.log(`🗑️ Removed tag from printer: ${printerName}`, matchingTag);
        }
      };
    });
  });
...
function populateTagListWrap(tagListWrap) {
  tagListWrap.innerHTML = ''; // Clear old content
  ...
  for (const [type, tagSet] of Object.entries(globalTagMap)) {
    Array.from(tagSet).sort().forEach(tagValue => {
      ...
      tagEl.dataset.tagType = type; // not shown, just for tracking
      tagEl.textContent = tagValue; // ✅ value only
