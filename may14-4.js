...
function initializeFilteredTab({ tabKey, filterFn }) {
  const filteredEntries = Array.from(fileDataMap.entries()).filter(filterFn);
  console.log(`${tabKey} files:`, filteredEntries);

  const tabPane = document.querySelector(`.w-tab-pane[data-w-tab="${tabKey}"]`);
  console.log(`${tabKey} tabPane:`, tabPane);
  const tabContainer = tabPane?.querySelector(".files-tabs-wrap");
  const paneContainer = tabPane?.querySelector(".file-info-content");
  const tabTemplate = document.getElementById("file-tab-template");
  const paneTemplate = document.getElementById("file-pane-template");

  if (!tabPane || !tabContainer || !paneContainer || !tabTemplate || !paneTemplate) {
    console.error(`❌ Missing containers or templates for ${tabKey} tab.`);
    return;
  }
  ...
  filteredEntries.forEach(([fileName, fileData]) => {
    ...
    pane.setAttribute("data-w-tab", fileName);
  ...
  if (window.Webflow?.require) {
    ...
    if (tabs && typeof tabs.ready === "function") {
      tabs.ready();
      console.log(`✅ Webflow tabs re-initialized for ${tabKey}`);
    }
  }
}
