...
function addTagToModal(displayTag, container) {
  console.log("addTagToModal");
  const exists = Array.from(container.querySelectorAll('.demo-tag.in-modal'))
    .some(el => el.style.display !== 'none' && el.textContent.trim() === displayTag);
  if (exists) return;
  ...
  document.querySelectorAll('.add-tag.printer').forEach(addTagBtn => {
    ...
    addTagBtn.addEventListener('click', e => {
      ...
      currentTagsWrap.onclick = (e) => {
        ...
        const matchingTag = printer.printerTags.find(t => t.split(":")[1] === tagValueOnly);

        if (matchingTag) {
          printer.printerTags = printer.printerTags.filter(t => t !== matchingTag);
          printer.printerTagsNum = printer.printerTags.length;
          printerDataMap.set(printerName, structuredClone(printer));
...
function removeTagFromFile(fileName, tagValueOnly) {
  ...
  const matchingTag = fileData.defaultTags.find(t => t.split(":")[1] === tagValueOnly);
  if (matchingTag) {
    fileData.defaultTags = fileData.defaultTags.filter(t => t !== matchingTag);
    fileDataMap.set(fileName, structuredClone(fileData));
    updateAllInstancesOfFile(fileName);
    console.log("🗑️ Removed tag from file:", matchingTag, fileData.defaultTags);
  }
}
  ...
  document.querySelectorAll('.add-tag.file').forEach(addTagBtn => {
    ...
    addTagBtn.addEventListener("click", e => {
      ...
      currentTagsWrap.onclick = (e) => {
        ...
        removeTagFromFile(fileName, tag);
        removeTagFromModal(tag, currentTagsWrap);
        updateDefaultTagsUI(fileTagsWrap, fileData.defaultTags);
