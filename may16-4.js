...
function updateAllInstancesOfFile(fileName) {
  console.log(`🌀 Updating ALL tab instances for: ${fileName}`);

  const tabSelectors = [
    `[data-tab-id="${fileName}"]`,
    `[data-w-tab="${fileName}"]`,
    `[data-tab-id="fav-${fileName}"]`,
    `[data-w-tab="fav-${fileName}"]`,
    `[data-tab-id="flag-${fileName}"]`,
    `[data-w-tab="flag-${fileName}"]`,
  ];

  const allTabs = tabSelectors.flatMap(selector => Array.from(document.querySelectorAll(selector)));
  console.log(`🔎 Found ${allTabs.length} matching tab(s)`);

  const fileData = fileDataMap.get(fileName);
  if (!fileData) {
    console.error(`❌ No fileData found for ${fileName}`);
    return;
  }
  ...
  allTabs.forEach((tab, idx) => {
    ...
    if (icon) {
      if (fileData.fileFav) {
        ...
        console.log(`⭐ Tab #${idx + 1} marked as favorited`);
      } else {
        icon.classList.remove("fav");
        console.log(`🚫 Tab #${idx + 1} unmarked as favorited`);
      }
    } else {
      console.warn(`❌ No icon found in tab #${idx + 1}`);
    }

    if (fileData.fileFlag) {
      tab.classList.add("flagged");
      console.log(`📌 Tab #${idx + 1} marked as flagged`);
    } else {
      tab.classList.remove("flagged");
      console.log(`🚫 Tab #${idx + 1} unmarked as flagged`);
    }

    // Final visual check
    console.log(`🔍 Final classList (Tab #${idx + 1}):`, [...tab.classList]);
    if (icon) {
      console.log(`🎯 Icon classes (Tab #${idx + 1}):`, [...icon.classList]);
    }
  });
}
