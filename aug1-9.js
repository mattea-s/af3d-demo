...
tagListWrap.querySelectorAll('.edit-tag-button.delete').forEach(btn => {
  attachDeleteListenerToButton(btn, modal);
});
    });
  });
}
...
document.getElementById('delete-tag-btn')?.addEventListener('click', () => {
  const deleteModal = document.getElementById('delete-tags-modal');
  const tagName = deleteModal.dataset.tagToDelete?.trim();
  if (!tagName) return;

  // 1. Remove from global tag repository
  tagRepoMap.delete(tagName);

  // 2. Remove from fileDataMap
  fileDataMap.forEach((data, key) => {
    if (data.defaultTags.includes(tagName)) {
      data.defaultTags = data.defaultTags.filter(t => t !== tagName);
      data.defaultTagsNum = data.defaultTags.length;
      fileDataMap.set(key, structuredClone(data));
      updateAllInstancesOfFile(key);
    }
  });

  // 3. Remove from jobDataMap
  jobDataMap.forEach((job, key) => {
    if (job.tags.includes(tagName)) {
      job.tags = job.tags.filter(t => t !== tagName);
      jobDataMap.set(key, structuredClone(job));
      const tagContainer = document.querySelector(`[data-job-id="${key}"] .default-file-tags.tags`);
      if (tagContainer) {
        updateDefaultTagsUI(tagContainer, job.tags);
      }
    }
  });

  // 4. Remove from printerDataMap
  printerDataMap.forEach((printer, key) => {
    if (printer.printerTags.includes(tagName)) {
      printer.printerTags = printer.printerTags.filter(t => t !== tagName);
      printer.printerTagsNum = printer.printerTags.length;
      printerDataMap.set(key, structuredClone(printer));

      // Avoid :has and :contains — use safe DOM traversal
      document.querySelectorAll('.demo-printer-card').forEach(card => {
        const nameEl = card.querySelector('.demo-printer-name');
        if (nameEl?.textContent.trim() === key) {
          updateDefaultPrinterTagsUI(card.querySelector('.default-file-tags'), printer.printerTags);
        }
      });
    }
  });

  // 5. Remove all tag-in-list-wrap elements that match this tag
  document.querySelectorAll('.tag-in-list-wrap').forEach(wrap => {
    const name = wrap.querySelector('.tag-name-in-list')?.textContent.trim();
    if (name && name === tagName) {
      wrap.remove();
    }
  });

  // 6. Clean up modal state
  deleteModal.classList.remove('show');
  deleteModal.dataset.tagToDelete = '';
  deleteModal.dataset.modalContextId = '';
  deleteModal.dataset.sourceWrapId = '';

  // 7. (Optional) Dispatch global event in case others need to listen
  document.dispatchEvent(new CustomEvent('tagDeleted', { detail: { tagName } }));

  // Re-attach delete listeners in all open modals
document.querySelectorAll('.add-tag-modal').forEach(modal => {
  modal.querySelectorAll('.edit-tag-button.delete').forEach(btn => {
    attachDeleteListenerToButton(btn, modal);
  });
});
});
...
function attachDeleteListenerToButton(btn, modal) {
  if (btn.dataset.listenerAttached === 'true') return;
  btn.dataset.listenerAttached = 'true';

  btn.addEventListener('click', function (e) {
    e.stopPropagation();
    const tagWrap = this.closest('.tag-in-list-wrap');
    const tagName = tagWrap?.querySelector('.tag-name-in-list')?.textContent.trim();
    if (!tagName) return;

    const deleteModal = document.getElementById('delete-tags-modal');
    deleteModal.classList.add('show');
    deleteModal.dataset.tagToDelete = tagName;
    deleteModal.dataset.modalContextId = modal.id || '';
    deleteModal.dataset.sourceWrapId = tagWrap.id || '';

    const tagDelText = deleteModal.querySelector('#tag-del');
    if (tagDelText) {
      tagDelText.textContent = tagName;
    }
  });
}
