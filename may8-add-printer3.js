...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function addNewPrinters(printers) {
    ...
    // Step 1: Count existing printers by model
    const modelCount = {};
    document.querySelectorAll(".demo-printer-card").forEach(card => {
      const modelNameEl = card.querySelector(".printer-card-model-name");
      const fullNameEl = card.querySelector(".demo-printer-name");
      ...
      if (modelName && fullName) {
        ...
        modelCount[base] = (modelCount[base] || 0) + 1;
      }
    });

    // Step 2: Add new printers
    printers.forEach(printer => {
      const baseModelName = printer.name;
      const startingCount = modelCount[baseModelName] || 0;
      const quantity = printer.quantity || 1; // Default to 1 for now
      ...
      for (let i = 0; i < quantity; i++) {
        ...
        if (tagContainer) {
          ...
          tagInner.className = "demo-tag-text printer";
          tagInner.textContent = fullPrinterName;
