...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (selectAllCheckbox) {
    selectAllCheckbox.addEventListener("change", function () {
      ...
      if (isChecked) {
        // Select all files in DOM order
        fileSelectionOrder = Array.from(document.querySelectorAll("a.file-tab")).map(tab => {
          return tab.querySelector(".file-text.filename")?.textContent.trim();
        });
      } else {
        fileSelectionOrder = [];
      }
