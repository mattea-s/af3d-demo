...
function initPrinterTagListeners() {
  ...
  // ✅ If no existing tag matches AND it's not in tagRepoMap, add it
  if (filter && !matchFound && !tagRepoMap.has(filter)) {
    console.log(`Creating new tag: "${filter}"`);

    // Add to tagRepoMap and printer
    ...
    // Trigger tag list refresh (your existing logic already handles deduping)
    tagSearch.value = '';
