...
// Initialize Files
function initializeFileData(limit = 10) {
  ...
  repoEntries.forEach(([fileName, repoData], index) => {
    ...
    const fileFavCheck = tab.querySelector(".fav-file-embed");
    ...
    if (fileWeightInput) fileWeightInput.setAttribute("value", repoData.filamentWeight);
    console.log(`${fileWeightInput} set to ${repoData.filamentWeight}`);
    if (fileTempInput) fileTempInput.setAttribute("value", repoData.defaultReleaseTemp);
    console.log(`${fileTempInput} set to ${repoData.defaultReleaseTemp}`);
    if (fileNotesText) fileNotesText.value = repoData.productionNotes;
    ...
    if (fileFlagCheck) {
      ...
      fileFlagCheck.addEventListener("change", () => {
        repoData.fileFlag = fileFlagCheck.checked;
        fileDataMap.set(fileName, structuredClone(repoData));
        console.log(`📌 File '${fileName}' flag updated to:`, repoData.fileFlag);
    ...
    [fileNotesText, fileWeightInput, fileTempInput, fileFlagCheck].forEach(input => {
  if (input) {
    input.addEventListener("input", () => {
      const wrap = input.closest(".file-info-wrap");
      wrap?.setAttribute("data-unsaved", "true");
    ...
    const saveBtn = tabPane.querySelector(".demo-button.file-save");
if (saveBtn) {
  saveBtn.addEventListener("click", () => {
    const wrap = saveBtn.closest(".file-info-wrap");
    const fileName = wrap.dataset.fileId;
    const fileData = fileDataMap.get(fileName);
    if (!fileData) return;

    fileData.productionNotes = wrap.querySelector(".production-notes")?.value || "";
    fileData.filamentWeight = parseInt(wrap.querySelector(".filament-weight")?.value) || 0;
    fileData.defaultReleaseTemp = parseInt(wrap.querySelector(".filament-weight.temp")?.value) || 29;
    fileData.fileFlag = wrap.querySelector(".flag-check")?.checked || false;

    fileDataMap.set(fileName, structuredClone(fileData));

    wrap.removeAttribute("data-unsaved");
    wrap.classList.remove("unsaved");

    console.log(`💾 Saved changes for ${fileName}`);
  });
}
...
function initializeFavoritesTab() {
  ...
  favorites.forEach(([fileName, fileData]) => {
    ...
    if (weightInput) weightInput.value = fileData.filamentWeight ?? "";

    const tempInput = pane.querySelector("input.filament-weight.temp");
    if (tempInput) tempInput.value = fileData.defaultReleaseTemp ?? "";

    const notesArea = pane.querySelector("textarea.production-notes");
    if (notesArea) notesArea.value = fileData.productionNotes ?? "";
...
function initPrinterTagListeners() {
  ...
  document.querySelectorAll('.add-tag.printer').forEach(addTagBtn => {
    ...
    // Clean current tags in modal
    currentTagsWrap.querySelectorAll(".demo-tag.in-modal:not([style*='display: none'])").forEach(tag => tag.remove());
    printer.printerTags.forEach(tag => addTagToModal(tag, currentTagsWrap));
    ...
    addTagBtn.addEventListener('click', e => {
      ...
      tagListWrap.querySelectorAll('.tag-name-in-list').forEach(tagEl => {
        const tag = tagEl.textContent.trim();
        const tagWrap = tagEl.closest('.tag-in-list-wrap');
        tagWrap.style.display = printer.printerTags.includes(tag) ? 'none' : 'flex';
...
function showSaveModal(onSave, onCancel) {
...
function initFileTagListeners() {
  ...
  document.querySelectorAll('.add-tag.file').forEach(addTagBtn => {
    ...
    addTagBtn.addEventListener('click', e => {
      ...
      // Show only unselected tags in the list
      tagListWrap.querySelectorAll('.tag-name-in-list').forEach(tagEl => {
        const tag = tagEl.textContent.trim();
        const tagWrap = tagEl.closest('.tag-in-list-wrap');
        tagWrap.style.display = fileData.defaultTags.includes(tag) ? 'none' : 'flex';
...
let currentFileTabID = null;

document.addEventListener("click", function (e) {
  const clickedTab = e.target.closest(".file-tab");
  if (!clickedTab || clickedTab.classList.contains("w-tab-link")) return;

  const nextTabID = clickedTab.dataset.tabId || clickedTab.dataset.wTab;
  if (!nextTabID || nextTabID === currentFileTabID) return;

  const currentPane = document.querySelector(`.file-info-tab[data-tab-id="${currentFileTabID}"]`);
  const unsavedWrap = currentPane?.querySelector(".file-info-wrap[data-unsaved='true']");

  if (unsavedWrap) {
    e.preventDefault();
    showSaveModal(() => {
      unsavedWrap.querySelector(".demo-button.file-save")?.click();
      simulateClick(clickedTab);
    }, () => {
      simulateClick(clickedTab);
    });
    return;
