...
document.addEventListener("DOMContentLoaded", function () {
  const selectedPrinters = new Map(); // Store selected printer data by name
  ...
  printerOptions.forEach(printerEl => {
    printerEl.addEventListener("click", () => {
      ...
      if (!isSelected) {
        ...
        selectedPrinters.set(printerModelName, { name: printerModelName });
        console.log(`✅ Selected: ${printerModelName}`);
      } else {
      ...
      addNewPrinters(Array.from(selectedPrinters.values()));
    });
  });
  ...
  function addNewPrinters(printers) {
    ...
    printers.forEach(printer => {
      const existing = container.querySelector(`[data-printer-name="${printer.name}"]`);
      if (existing) return; // Skip duplicates

      const clone = template.cloneNode(true);
      clone.removeAttribute("id");
      clone.style.display = "flex";
      clone.setAttribute("data-printer-name", printer.name);

      const nameEl = clone.querySelector(".printer-card-model-name");
      if (nameEl) nameEl.textContent = printer.name;

      container.appendChild(clone);

      // ✅ Register in printerDataMap so it can receive jobs
      if (!printerDataMap.has(printer.name)) {
        printerDataMap.set(printer.name, {
          printerName: printer.name,
          printerModel: "",
          printerTags: [],
          printerTagsNum: 0,
          filamentRemaining: 1000, // Default to 1kg
          currentJob: null,
          compatibleJobs: [],
          printerStatus: "idle"
        });
        console.log(`🆕 Added ${printer.name} to printerDataMap.`);
      }

      // ✅ Trigger tag listener + compatibility refresh
      initPrinterTagListeners(); // Rebinds modal editing to this new printer
      updateAllJobMatches();     // Refresh job ↔ printer compatibility
    });
  }
});
