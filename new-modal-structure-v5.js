document.addEventListener("DOMContentLoaded", function () {
  ...
  fileItems.forEach(item => {
    ...
    const qtyInput = item.querySelector(".filament-weight-wrap.q-qty input");
    const tempInput = item.querySelector(".filament-weight-wrap.release-temp input");
    const distCheckbox = item.querySelector(".distribute-checkbox");

    const job = Array.from(jobDataMap.values()).find(j => j.fileName === fileName);

    if (job) {
      if (qtyInput) job.quantity = parseInt(qtyInput.value) || 1;
      if (tempInput) job.releaseTemp = parseInt(tempInput.value) || 29;
      if (distCheckbox) job.distribute = distCheckbox.checked;
