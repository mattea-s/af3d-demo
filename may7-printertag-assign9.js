...
function renderQueuedJob(job) {
  ...
  const existingDiv = document.querySelector(`.w-tab-pane[data-w-tab="Queued"] [data-job-id="${job.jobID}"]`);
  if (existingDiv) {
    console.warn(`⚠️ Job ${job.jobID} already exists in Queued. Skipping append.`);
    return;
  }
