...
function initializeFavoritesTab() {
  ...
  favorites.forEach(([fileName, fileData]) => {
    ...
    tab.querySelector(".material-symbols-outlined")?.addEventListener("click", (e) => {
      ...
      initializeFavoritesTab();
      console.log("initializeFavoritesTab(); called from Fav");
      initializeAllFilesTab();
      console.log("initializeAllFilesTab(); called from Fav");
      initializeFlaggedTab();
      console.log("initializeFlaggedTab(); called from Fav");
    });
...
function initializeAllFilesTab() {
  ...
  allFiles.forEach(([fileName, fileData]) => {
    ...
    tab.style.display = "flex";
    tab.classList.add("file-tab");
    tab.setAttribute("data-tab-id", `all-${fileName}`);
    tab.setAttribute("id", `all-${fileName}`);
    ...
    pane.style.display = "";
    pane.classList.add("file-info-tab");
    pane.setAttribute("data-tab-id", `all-${fileName}`);
...
function updateAllInstancesOfFile(fileName) {
  ...
  // Update all .file-tab instances
document.querySelectorAll(`.file-tab[id$="${fileName}"]`).forEach(tab => {
  console.log(`🔍 Updating tab with ID: ${tab.id}`);

  const favIcon = tab.querySelector(".material-symbols-outlined");
  if (favIcon) {
    const shouldBeFav = data.fileFav;
    favIcon.classList.toggle("fav", shouldBeFav);
    console.log(`⭐ favIcon ${shouldBeFav ? "added" : "removed"} for: ${fileName}`);
  } else {
    console.warn(`❌ No favIcon found in tab ID: ${tab.id}`);
  }

  if (data.fileFlag) {
    tab.classList.add("flagged");
    console.log(`📌 Tab ${tab.id} marked as flagged`);
  } else {
    tab.classList.remove("flagged");
    console.log(`🚫 Tab ${tab.id} unmarked as flagged`);
  }
});

  // Update all .file-info-tab instances
  document.querySelectorAll(`.file-info-tab[data-tab-id$="${fileName}"]`).forEach(pane => {
    const flagCheck = pane.querySelector("input.flag-check");
    if (flagCheck) flagCheck.checked = data.fileFlag;

    const notes = pane.querySelector("textarea.production-notes");
    if (notes) notes.value = data.productionNotes;

    const weight = pane.querySelector("input.filament-weight:not(.temp)");
    if (weight) weight.value = data.filamentWeight;

    const temp = pane.querySelector("input.filament-weight.temp");
    if (temp) temp.value = data.defaultReleaseTemp;

    const tagContainer = pane.querySelector(".default-file-tags");
    if (tagContainer) updateDefaultTagsUI(tagContainer, data.defaultTags);
  });
}
