...
function initializeFileData(limit = 10) {
  ...
  repoEntries.forEach(([fileName, repoData], index) => {
    ...
    tab.querySelector(".material-symbols-outlined")?.addEventListener("click", (e) => {
      ...
      initializeFilteredTab({
        tabKey: "Fav",
        filterFn: ([_, file]) => file.fileFav
      });
    ...
    if (fileFlagCheck) {
      ...
      fileFlagCheck.addEventListener("change", () => {
        ...
        fileDataMapRepo.set(fileName, structuredClone(repoData));
...
function initializeFavoritesTab() {
  ...
  const tabTemplate = document.getElementById("fav-file-tab-template");
  const paneTemplate = document.getElementById("fav-file-pane-template");
  ...
  favorites.forEach(([fileName, fileData]) => {
    ...
    tab.style.display = "flex";
    tab.classList.add("file-tab", "w-tab-link");
    tab.setAttribute("data-w-tab", `fav-${fileName}`);
    tab.setAttribute("id", `fav-${fileName}`);

    const fileTabNameText = tab.querySelector(".file-text.filename");
    if (fileTabNameText) fileTabNameText.textContent = fileName;
    tab.querySelector(".fav-file-embed")?.classList.add("fav");
    
    tabContainer.appendChild(tab);

    pane.removeAttribute("id");
    pane.style.display = "";
    pane.classList.add("w-tab-pane", "file-info-tab");
    pane.setAttribute("data-w-tab", `fav-${fileName}`);
  ...
  if (window.Webflow && Webflow.require) {
    ...
    if (tabs && typeof tabs.ready === "function") {
      tabs.ready();
      console.log("✅ Webflow tabs re-initialized for Favorites");
    }
  }
}
...
function initializeFlaggedTab() {
  ...
  flagged.forEach(([fileName, fileData]) => {
    ...
    tab.style.display = "flex";
    tab.classList.add("file-tab", "w-tab-link");
    tab.setAttribute("data-w-tab", `flag-${fileName}`);
    ...
    pane.style.display = "";
    pane.classList.add("w-tab-pane", "file-info-tab");
    pane.setAttribute("data-w-tab", `flag-${fileName}`);
...
document.addEventListener("click", function (e) {
  ...
  const tabID = clickedTab.dataset.wTab; // like "flag-Articulated-Dragon"
  if (!tabID) return;
  ...
  // Hide all .file-info-tabs
  document.querySelectorAll(".file-info-tab").forEach(pane => {
  ...
  const matchingPane = document.querySelector(`.file-info-tab[data-w-tab="${tabID}"]`);
  if (matchingPane) {
